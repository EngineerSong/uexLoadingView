...
- (void)addCircles {
    for (NSUInteger i = 0; i < self.numberOfCircles; i++) {
        ...
        if (i < self.colorArr.count) {
            customColorStr = [self.colorArr objectAtIndex:i];
            customColor = [EUtility ColorFromString:customColorStr];
        }
        UIView *circle = [self createCircleWithRadius:self.radius
                                                color:(customColor == nil) ? color : customColor
